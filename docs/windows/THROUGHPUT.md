...
Waitable swap chain (`DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT`) + `SetMaximumFrameLatency(1)` and wait before recording/presenting.
...
Viewport scroll: avoid `dirty = .full` on pin change; rotate/reuse rows and dirty only newly exposed rows; optional DX12 row-range buffer sync.