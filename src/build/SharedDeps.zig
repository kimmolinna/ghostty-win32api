...
pub fn add(
    ...
    if (step.kind != .lib) {
        // OpenGL glad is only needed for the OpenGL renderer (GTK Linux etc.).
        // Win32 uses DX12 and does not need glad.
        if (self.config.app_runtime != .win32) {
...
pub fn addSimd(
    ...
    m.addIncludePath(b.path("src"));
    {
        ...
        // codepoint_width.cpp intentionally omitted: production width uses
        // the Zig unicode LUT; simd/codepoint_width.zig uses uucode.
        m.addCSourceFiles(.{
            .files = &.{
                "src/simd/base64.cpp",
