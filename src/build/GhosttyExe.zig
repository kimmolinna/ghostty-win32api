...
pub fn init(b: *std.Build, cfg: *const Config, deps: *const SharedDeps) !Ghostty {
    ...
    switch (cfg.target.result.os.tag) {
        .windows => {
            // Console subsystem: Zig entry is `main`, not WinMain. GUI
            // subsystem requires a WinMain/wWinMain export that Zig+MSVC
            // does not currently emit for this target. Console still
            // creates normal top-level windows; AllocConsole is unused.
